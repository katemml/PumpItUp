...
#labels are in last column/attribute (must be moved to seperate file before running in classifier code)
...
train_value = pd.read_csv("train_data_with_labels.csv")
...
train_value.dropna(axis = 0 , how = 'any' , inplace = True)
#test_value.dropna(axis = 0 , how = 'any' , inplace = True)
...
train_value.to_csv("cleaned_train_values_5.csv" , index = False)

#test_value.to_csv("cleaned_test_values_4.csv" , index = False)
